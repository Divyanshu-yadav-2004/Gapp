...
let EmailService = EmailService_1 = class EmailService {
    ...
    async sendEmail(to, subject, htmlContent) {
        const fromName = this.configService.get('SMTP_FROM_NAME', 'EasyCafe');
        const fromEmail = this.configService.get('SMTP_FROM_EMAIL', 'noreply@easycafe.com');
        const maxRetries = 3;
        let attempt = 0;
        let success = false;
    ...
    async sendWelcomeAndSubmissionEmail(email, customerName, applicationId, serviceName, amountPaid, timeline) {
    ...
    async sendAdminNotificationOfNewSubmission(adminEmail, customerName, applicationId, serviceName, amountPaid) {
    ...
    async sendAdminPaymentSuccessNotification(adminEmail, customerName, serviceName, amountPaid, transactionId, status) {
    ...
    async sendStatusChangeEmail(email, customerName, applicationId, serviceName, oldStatus, newStatus, comment, certificateNumber) {
    ...
    async sendUserPaymentReceiptConfirmedEmail(email, customerName, amount, applicationId) {
    ...
    async sendAdminPaymentReceiptNotificationEmail(adminEmail, customerName, customerEmail, amount, plan, paymentTime, whatsappStatus) {
